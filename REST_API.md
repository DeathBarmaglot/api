##level 1:
####У нас есть следущия сущность.
• Пост -> id, title, content
...
DELETE /api/v1/posts/{id} - удаление поста по id
Используем Spring boot + Spring Data Jpa
##level 2:
...
###level 4:
Добавляется сущность комментарий (на данный момент аннонимеых)
id, text, creationDate(не передается с клиента а генерируется в приложении), postId
REST запросы для работы с комментами
POST /api/v1/posts/1/comments
GET /api/v1/posts/{id}/comments
GET /api/v1/posts/{postId}/comment/{commentId}
###level 5:
GET /api/v1/posts/1/full - возвращает JSON поста с ид = 1, 
и всеми вложенными комментариями в него